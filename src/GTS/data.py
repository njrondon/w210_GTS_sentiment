...
class GTSPreprocessor:
    ...
    def __init__(self,
        ...
        if mode != 'predict':
            ...
            self.holder_tags = torch.zeros(max_sequence_len).long() # JR
    ...
    def initialize(self):
        ...
        if self.mode != 'predict':
            ...
            self.holder_tags[self.len_plm_tokens:] = -1 # padding
            self.holder_tags[0] = -1
            self.holder_tags[self.len_plm_tokens-1] = -1 # [SEP]
            ...
            for opinion in self.sentence_pack['opinions']:
                ...
                if not holder_span and aspect_span:  # JR
                    holder_span = [[-1, -1]]
                ...
                '''set tag for holder JR'''
                for l, r in holder_span:
                    ...
                    for i in range(l, r+1):
                        ...
                        hl, hr = self.token_range[i+1]
                        self.holder_tags[hl] = set_tag
                        self.holder_tags[l+1:hr+1] = -1
                        '''mask positions of sub words'''
                        self.tags[hl+1:hr+1, :] = -1
                        self.tags[:, hl+1:hr+1] = -1
                ...
                '''tag the holder with the opinion JR'''
                for hl, hr in holder_span:
                    for pl, pr in opinion_span:
                        for i in range(hl, hr+1):
                            for j in range(pl, pr+1):
                                shl, shr = self.token_range[i+1]
                                spl, spr = self.token_range[j+1]
                                self.tags[shl:shr+1, spl:spr+1] = -1
                                if i > j:
                                    self.tags[spl][shl] = sentiment2id[opinion['Polarity'].lower()]
                                else:
                                    self.tags[shl][spl] = sentiment2id[opinion['Polarity'].lower()]
