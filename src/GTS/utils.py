...
def convert_to_submission_json_format(sentence_pack, tuples, mode='quadtuple'):
...
class OutputPreprocessor:
    ...
    def find_triplet(self, tags, aspect_spans, opinion_spans, token_ranges):
     ...
     #JR this function was added
    def find_quadplet(self, tags, holder_spans, aspect_spans, opinion_spans, token_ranges):

        quadplets = []
        merged_spans =[]

        if len(holder_spans) == 0:
            merged_spans = [h + a for h, a in zip(aspect_spans, aspect_spans)]
        elif any(not sublist for sublist in holder_spans):
            merged_spans= [sublist if sublist else aspect for sublist, aspect in zip(holder_spans, aspect_spans)]
        elif len(holder_spans)  != len(aspect_spans):
            merged_spans = [ holder_spans[0] + span for span in aspect_spans]   
        else:
            merged_spans = [h + a for h, a in zip(holder_spans, aspect_spans)]   

        for hl, hr, al, ar in merged_spans:
            for pl, pr in opinion_spans:
                tag_num = [0] * 10
                for i in range(al, ar + 1):
                    for j in range(pl, pr + 1):
                ...
                if sentiment == -1:
                    print('wrong!!!!!!!!!!!!!!!!!!!!')
                quadplets.append([hl, hr, al, ar, pl, pr, sentiment])
        return quadplets
    ...
    def get_tuples(self):
        ...
        predicted_tuples = []
        predicted_quadplets =[] # JR

        for i in range(self.data_num):
            # additions JR
            predicted_holder_spans = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 8)
            implict_holder = self.get_spans(self.predictions[i], self.sen_lengths[i], self.tokens_ranges[i], 9)
            predicted_holder_spans.extend(implict_holder)
            ...
            #added JR
            predicted_quadplets.append(self.find_quadplet(self.predictions[i],predicted_holder_spans, predicted_aspect_spans, predicted_opinion_spans, self.tokens_ranges[i]))

        return predicted_tuples , predicted_quadplets # JR
