...
def train(args, device):
    ...
    for epoch in prange(int(args.epochs), desc="Epoch", disable=args.disable_progress_bar):
        ...
        predicted_tuples, _, _, f1, predicted_quadtuples = evaluate(model, valid_dataloader, device, args) #JR
        ...
        predicted_json = convert_to_submission_json_format(valid_sentence_packs, predicted_quadtuples)
...
def evaluate(model, dataloader, device, args, mode='valid'):
    ...
    output_preprocessor = OutputPreprocessor(
        ...
        all_token_ranges, 
        ignore_index=-1)
    predicted_tuples, predicted_quadplets = output_preprocessor.get_tuples() # JR

    #semeval_evaluator = SemEvalEvaluator(predicted_tuples, args.dataset, mode)
    semeval_evaluator = SemEvalEvaluator(predicted_quadplets, args.dataset, mode) # JR
    ...
    return predicted_triplets, precision, recall, f1 , predicted_quadtuples
...
def test(args, device):
    ...
    predicted_triplets, _, _, _,predicted_quadtuples = evaluate(model, test_dataloader, device, args, mode='test') #JR

    predicted_json = convert_to_submission_json_format(test_sentence_packs, predicted_quadtuples) #JR changed to quads
    
    pred_save_dir = model_save_dir / 'extract' / f"{args.plm_model_name}" / args.dataset
    with open(pred_save_dir / f"debuggin.json", "w+") as f_json:
            json.dump(predicted_json, f_json)
...
def parse_args():
    ...
    parser.add_argument('--plm_model_name',
                        ...
                        choices=["bert-base-uncased", "xlm-roberta-large", "ernie_2.0_skep_large_en_pytorch", \
                            ...
                            "nb-bert-large", "bert-large-uncased-domain-pretrained-only-europarl", "LaBSE", "BERT_Review", "bert-base-multilingual-uncased", "distilbert-base-multilingual-cased"],
